...
def format_claude_status(payload: dict[str, Any], use_color: bool = False) -> str:
    ...
    if env_flag("KT_STATUSLINE_SHOW_COST"):
        ...
        if cost is not None:
            parts.append(colorize(f"${cost:.4f}", DIM, use_color))
    if env_flag("KT_STATUSLINE_SHOW_VERSION"):
        version = first_text(payload.get("version"))
        if version:
            parts.append(colorize(f"cc {version}", DIM, use_color))
    return " · ".join(parts)
